...
pnts = switch(pnts_name,
    ...
    no_action = {
      ...
      # mapview(bbox) + mapview(landscape_planning_units_clean) + mapview(grid)
    },
...
if (overwrite_data_homerange_scale_cache) {
  ...
  for (i in 1:nrow(homeranges)) {
    ...
    for (j in 1:nrow(pnts)) {
      ...
      # focal_patch_and_edge_buffer = trim(classify(hr_patch_ids, cbind(pid, 1), others = NA))
      ...
      # (focal_cover_class = pnt$cover_class)
      # matching_cover = rast_cover
