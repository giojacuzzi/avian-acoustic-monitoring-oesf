...
path_out = paste0("data/cache/models/msom_", model_name, "_", model_thresh, "_", format(Sys.Date(), "%Y-%m-%d"), ".rds")
...
species_thresholds_target = species_thresholds %>% filter(model == 'target') %>% filter(species %in% species_thresholds_source$species)
...
if (model_thresh == "source") {
  species_thresholds_manual_selection = species_thresholds_source
} else if (model_thresh == "target") {
  species_thresholds_manual_selection = rbind(species_only_in_source, species_thresholds_target)
}
