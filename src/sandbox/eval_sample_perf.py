...
# DEBUG ######################
# preexisting_labels_to_evaluate = ['Colaptes auratus_Northern Flicker']
# target_labels_to_evaluate = ['Colaptes auratus_Northern Flicker']
# all_labels = ['Colaptes auratus_Northern Flicker']
# DEBUG ######################
    ...
    for model in models:
        ...
        if False:
            ...
            for label_under_evaluation in model_labels_to_evaluate:
                ...
                ## DEBUG ##
                # y_prob = model.predict_proba(model_x)[:, 1]  # probability of positive class
                # print(f'y_prob {y_prob}')
                # negatives = model_y == 0 # find all negative examples
                # false_positive_rate = np.mean(y_prob[negatives] >= logit_to_conf(p_threshold))
                # print(f"False positive rate at threshold: {false_positive_rate}")
                ###########
                ...
                # Plot data points
                # plt.scatter(model_x, model_y, edgecolors="blue", facecolors="none", label="Prediction")
                ...
                ## DEBUG ##
                # neg_model_x = detection_labels[["score"]]
                # neg_model_y = detection_labels["label_truth"].replace({0: 1, 1: 0})
                # neg_model = LogisticRegression()
                # neg_model.fit(neg_model_x, neg_model_y)
                # neg_intercept = neg_model.intercept_[0]  # Intercept (bias term)
                # neg_coefficient = neg_model.coef_[0][0]  # Coefficient for 'score'
                # prob_TN = 1 / (1 + np.exp(-(neg_intercept + neg_coefficient * p_threshold)))
                # print(f'prob_TN {prob_TN}')
                # neg_x_range = pd.DataFrame({"score": np.linspace(neg_model_x["score"].min(), neg_model_x["score"].max(), 100)})  # Range of scores
                # neg_y_pred = neg_model.predict_proba(neg_x_range)[:, 1]  # Predicted probabilities (positive class)
                # plt.scatter(neg_model_x[neg_model_y == 0], np.zeros_like(neg_model_x[neg_model_y == 0]), marker='_', color='blue', label="Prediction (0)")
                # plt.scatter(neg_model_x[neg_model_y == 1], np.ones_like(neg_model_x[neg_model_y == 1]), marker='+', color='blue', label="Prediction (1)")
                # plt.plot(neg_x_range, neg_y_pred, color="red", label="Regression")
                # plt.axvline(x=p_threshold, color="black", linestyle="--", linewidth=1, label=f"Threshold {np.round(logit_to_conf(p_threshold), 2)} --> p(TN)~{prob_TN}")
                # plt.xlabel("Score (logit scale)")
                # plt.ylabel("Probability")
                # plt.title(f"{label_under_evaluation}")
                # plt.legend(loc="lower right")
