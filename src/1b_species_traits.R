...
avonet = readxl::read_xlsx('data/traits/AVONET Supplementary dataset 1.xlsx', sheet = "AVONET2_eBird") %>%
  janitor::clean_names() %>% rename(scientific_name = species2, family = family2, order = order2)
...
setdiff(metadata$scientific_name, avonet$scientific_name)
avonet = avonet %>% mutate(scientific_name = if_else(scientific_name == "Accipiter gentilis", "Astur atricapillus", scientific_name))
avonet = avonet %>% mutate(scientific_name = if_else(scientific_name == "Glaucidium gnoma", "Glaucidium californicum", scientific_name))
avonet = avonet %>% mutate(scientific_name = if_else(scientific_name == "Tyto alba", "Tyto furcata", scientific_name))
setdiff(metadata$scientific_name, avonet$scientific_name)

# Join metadata with AVONET, discarding irrelevant data
trait_data = left_join(metadata, avonet, by = "scientific_name") %>% select(common_name, scientific_name, family, order, habitat, habitat_density, migration, trophic_level, trophic_niche, primary_lifestyle, residency, hand_wing_index, mass, home_range_radius_m) %>% arrange(common_name)
...
trait_data = trait_data %>% left_join(predicted_home_range_by_both %>% select(common_name, predicted, lower, upper), by = "common_name") %>%
  mutate(home_range_radius_m = ifelse(is.na(home_range_radius_m), predicted, home_range_radius_m))

trait_data$range_bin = cut(trait_data$home_range_radius_m,
...
ggplot(trait_data, aes(x = "",y = home_range_radius_m)) +
  geom_violin() +
