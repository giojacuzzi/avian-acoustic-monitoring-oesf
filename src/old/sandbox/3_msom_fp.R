...
#
# False-positive model formulated according to Royle-Link 2006 binary false-positive model
#
...
detection_prob_v = msom_summary %>% filter(stringr::str_starts(param, "v")) %>% arrange(mean) %>% mutate(plot_order = 1:nrow(.)) %>%
  mutate(species_idx = as.integer(gsub(".*\\[(\\d+)\\]", "\\1", param))) %>% mutate(species_name = species[species_idx])
mean_detection_prob_v = msom_summary %>% filter(param == "mu.v")
message("Mean species TP detection probability: ", round(mean_detection_prob_v$prob,2), " (95% BCI ", round(mean_detection_prob_v$prob_lower95,2), "–", round(mean_detection_prob_v$prob_upper95,2), ")")
message("Species TP detection probability range: ", round(min(detection_prob_v$prob),2), "–", round(max(detection_prob_v$prob),2), " (", detection_prob_v %>% slice_min(prob, n=1) %>% pull(species_name), ", ", detection_prob_v %>% slice_max(prob, n=1) %>% pull(species_name), ")")
ggplot(detection_prob_v, aes(x = as.factor(plot_order), y = prob)) +
  geom_hline(yintercept = mean_detection_prob_v$prob,         linetype = "solid", color = "blue") +
  geom_hline(yintercept = mean_detection_prob_v$prob_lower95, linetype = "dashed", color = "blue") +
  geom_hline(yintercept = mean_detection_prob_v$prob_upper95, linetype = "dashed", color = "blue") +
  geom_point() + geom_errorbar(aes(ymin = `prob_lower95`, ymax = `prob_upper95`), width = 0) +
  scale_x_discrete(labels = detection_prob_v$species_name) + 
  scale_y_continuous(limits = c(0.0, 1.0), breaks = c(0, 0.25, 0.5, 0.75, 1.0)) +
  labs(title = "Baseline TP detection probability", x = "Species", y = "Detection probability") +
  coord_flip()

# 
detection_prob_fp = msom_summary %>% filter(stringr::str_starts(param, "fp")) %>% arrange(mean) %>% mutate(plot_order = 1:nrow(.)) %>%
  mutate(species_idx = as.integer(gsub(".*\\[(\\d+)\\]", "\\1", param))) %>% mutate(species_name = species[species_idx])
mean_detection_prob_fp = msom_summary %>% filter(param == "mu.fp")
message("Mean species FP detection probability: ", round(mean_detection_prob_fp$prob,2), " (95% BCI ", round(mean_detection_prob_fp$prob_lower95,2), "–", round(mean_detection_prob_fp$prob_upper95,2), ")")
message("Species FP detection probability range: ", round(min(detection_prob_fp$prob),2), "–", round(max(detection_prob_fp$prob),2), " (", detection_prob_fp %>% slice_min(prob, n=1) %>% pull(species_name), ", ", detection_prob_fp %>% slice_max(prob, n=1) %>% pull(species_name), ")")
ggplot(detection_prob_fp, aes(x = as.factor(plot_order), y = prob)) +
  geom_hline(yintercept = mean_detection_prob_fp$prob,         linetype = "solid", color = "blue") +
  geom_hline(yintercept = mean_detection_prob_fp$prob_lower95, linetype = "dashed", color = "blue") +
  geom_hline(yintercept = mean_detection_prob_fp$prob_upper95, linetype = "dashed", color = "blue") +
  geom_point() + geom_errorbar(aes(ymin = `prob_lower95`, ymax = `prob_upper95`), width = 0) +
  scale_x_discrete(labels = detection_prob_fp$species_name) + 
  scale_y_continuous(limits = c(0.0, 1.0), breaks = c(0, 0.25, 0.5, 0.75, 1.0)) +
  labs(title = "Baseline FP detection probability", x = "Species", y = "Detection probability") +
  coord_flip()

# Which species have greater probability of false positive than true positive detection? These confuse the mixture model!
detection_prob_v %>%
  select(species_idx, species_name, prob_v = prob, prob_v_lower95 = prob_lower95, prob_v_upper95 = prob_upper95) %>%
  inner_join(
    detection_prob_fp %>% select(species_idx, prob_fp = prob, prob_fp_lower95 = prob_lower95, prob_fp_upper95 = prob_upper95),
    by = "species_idx"
  ) %>%
  mutate(fp_greater_than_v = prob_fp > prob_v) %>% filter(fp_greater_than_v)
