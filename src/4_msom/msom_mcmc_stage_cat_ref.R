## 2_msom_mcmc.R #########################################################################################
# Fit a multispecies occupancy model using MCMC with stage as a categorical variable, using compex as the baseline reference
#
## CONFIG:
grouping = "nest" # Species grouping ("all", "diet", "nest" ...)
model_type = "nofp" # nofp, fp
param_alpha_stage = "stratum_4" # stage_3, stratum_4
...
path_occurrence_predictor_plot_data      = "data/cache/4_msom/1_assemble_msom_data/NEW_occurrence_predictor_plot_data.rds"
path_occurrence_predictor_homerange_data = "data/cache/4_msom/1_assemble_msom_data/NEW_occurrence_predictor_homerange_data.rds"
path_detection_predictor_data            = "data/cache/4_msom/1_assemble_msom_data/NEW_detection_predictor_data.rds"
...
if (model_type == "nofp") {
  model_file = "src/4_msom/msom_stage_cat_ref.txt"
...
path_out = paste0(dir_out, "/", model_name, "_", model_type, "_", grouping, "_", param_alpha_stage, ".rds")
...
# Stage categorical
if (param_alpha_stage == "stage_3") {
  stages = occurrence_predictor_plot_data %>% select(stage_3) %>% mutate(stage_idx = as.integer(stage_3))
  # NOTE: REDEFINE COMPEX AS IDX 1, FOR USE AS BASELINE STAGE
  stages$stage_idx = as.numeric(
    factor(stages$stage_3,
           levels = c("compex", "standinit", "mature"))
  )
} else if (param_alpha_stage == "stratum_4") {
  stages = occurrence_predictor_plot_data %>% select(stratum_4) %>% mutate(stage_idx = as.integer(as.factor(stratum_4)))
  # NOTE: REDEFINE COMPEX AS IDX 1, FOR USE AS BASELINE STAGE
  stages$stage_idx = as.numeric(
    factor(stages$stratum_4,
           levels = c("compex", "standinit", "mature", "thin"))
  )
}
...
# DEBUG: stage_3
path_out = "data/cache/models/msom_stage_cat_ref_nofp_nest.rds" # OR "data/cache/models/msom_stage_cat_ref_nofp_nest_stratum_4.rds"
...

{
  ...
  ggplot(mu_alpha_stage, aes(x = mean, y = fct_rev(stratum_4), color = group)) +
  ...
  ggplot(alpha_stage %>% filter(stratum_4 == "standinit") %>% mutate(species = fct_reorder(species, mean)), aes(x = mean, y = species, color = group)) +
    geom_vline(xintercept = 0) + geom_point(position = position_dodge(width = 0.5), size = 2) +
    geom_errorbarh(aes(xmin = `2.5%`, xmax = `97.5%`), height = 0.1, position = position_dodge(width = 0.5)) +
  ...
  ggplot(alpha_stage %>% filter(stratum_4 == "mature") %>% mutate(species = fct_reorder(species, mean)), aes(x = mean, y = species, color = group)) +
    geom_vline(xintercept = 0) + geom_point(position = position_dodge(width = 0.5), size = 2) +
    geom_errorbarh(aes(xmin = `2.5%`, xmax = `97.5%`), height = 0.1, position = position_dodge(width = 0.5)) +
    labs(title = "mature")
  
  ggplot(alpha_stage %>% filter(stage_3 == "thin") %>% mutate(species = fct_reorder(species, mean)), aes(x = mean, y = species, color = group)) +
    geom_vline(xintercept = 0) + geom_point(position = position_dodge(width = 0.5), size = 2) +
    geom_errorbarh(aes(xmin = `2.5%`, xmax = `97.5%`), height = 0.1, position = position_dodge(width = 0.5)) +
    labs(title = "thin")
