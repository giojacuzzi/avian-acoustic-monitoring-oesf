...
# TODO: Calculate PPC directly from the posterior samples
{
  str(msom$sims.list$pi) # Expected: num [1:n.samples, 1:J, 1:Kmax, 1:Tseason, 1:I, 1:3]
  
  n.samples <- msom$mcmc.info$n.samples
  pi_post   <- msom$sims.list$pi  # [n.samples, J, Kmax, Tseason, I, 3]
  
  D_obs_post <- numeric(n.samples)
  D_sim_post <- numeric(n.samples)
      ...
      for (t in 1:Tseason) {
        for (j in 1:J) {
          for (k in 1:K[j,t]) {
            ...
            # Observed deviance
            pi_obs  <- pi_post[s, j, k, t, i, y[j,k,t,i]]
            d_obs_s <- d_obs_s + log(max(pi_obs, 1e-10))
            
            # Simulated deviance
            pi_vec  <- pi_post[s, j, k, t, i, ]
            y_sim   <- sample(1:3, size = 1, prob = pi_vec)
            d_sim_s <- d_sim_s + log(max(pi_vec[y_sim], 1e-10))
          }
        }
      }
    }
    
    D_obs_post[s] <- -2 * d_obs_s
    D_sim_post[s] <- -2 * d_sim_s
  }
  close(pb)
  
  # Bayesian p-value — should be near 0.5 for a well-fitting model
  ppc_p <- mean(D_sim_post > D_obs_post)
  cat("DEBUG Bayesian p-value:", round(ppc_p, 3), "\n")
  
  # Plot
  plot(D_obs_post, D_sim_post,
       pch  = 16, cex = 0.3, asp = 1,
       xlab = "D_obs", ylab = "D_sim",
       main = paste0("PPC (Bayesian p = ", round(ppc_p, 3), ")"))
  abline(0, 1, col = "red")
}
