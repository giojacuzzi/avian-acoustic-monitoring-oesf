...
plot_gradient_concat_multi <- function(pred_df,
  ...
  # Build line segments: stage-gradient color blended 70/30 with species color
  line_segs <- pred_df |>
    group_by(sp_name) |>
    mutate(
      x_end       = lead(x),
      y_end       = lead(psi_mean),
      color_next  = lead(hex_color),
      stage_mid   = map2_chr(hex_color, color_next,
                             \(a, b) if (is.na(b)) NA_character_
                             else interpolate_colors(a, b, t = 0.5)),
      sp_color    = sp_colors[sp_name],
      color_final = map2_chr(stage_mid, sp_color,
                             \(s, p) if (is.na(s)) NA_character_
                             else interpolate_colors(s, p, t = 0.3))
    ) |>
    ungroup() |>
    filter(!is.na(x_end), !is.na(color_final))
  ...
  # Species labels positioned at the right edge, spaced vertically
  legend_df <- tibble(
    sp_name = sp_names,
    color   = sp_colors[sp_names],
    y       = seq(0.95, 0.95 - (n_sp - 1) * 0.07, length.out = n_sp)
  )
  ...
  ggplot() +
    ...
    geom_text(
      data        = legend_df,
      mapping     = aes(x = 3.02, y = y, label = sp_name, color = color),
